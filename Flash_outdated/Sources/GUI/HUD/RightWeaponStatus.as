...
function onLoad() 
{
	//This is what makes this the right weapon
	//almost all other code should be done in WeaponStatus, as it will be the same for the left weapon!
	m_WeaponSlot = _global.Enums.ItemEquipLocation.e_Wear_Second_WeaponSlot;
...
{	
	...
	if (edit)
	{
		...
		{
			var scaleDV:DistributedValue = DistributedValue.Create("RightWeaponStatusScale");
...
function SlotEditMaskPressed()
{
	...
	scale *= DistributedValue.GetDValue("RightWeaponStatusScale", 100) / 100;
...
function SlotEditMaskReleased()
{
	...
	var newX:DistributedValue = DistributedValue.Create( "RightWeaponStatusX" );
	var newY:DistributedValue = DistributedValue.Create( "RightWeaponStatusY" );	
