...
import com.GameInterface.PendingRewardItemData;
...
// Class
class GUI.Claim.PvPSpoilsContent extends WindowComponentContent
{
    //Constnats
	...
	private function configUI():Void
    {
        ...
        SlotClaimsUpdated();
        PendingReward.SignalClaimsUpdated.Connect(SlotClaimsUpdated, this);
    ...
    private function SlotClaimsUpdated():Void
    {
        ...
        {
            var item:MCLItemInventoryItem = new MCLItemInventoryItem(PendingReward.m_Claims[i].m_InventoryItem, undefined);
            item.SetId(PendingReward.m_Claims[i].m_ID);
            item.m_Recurrent = PendingReward.m_Claims[i].m_MaxClaims == -1 || PendingReward.m_Claims[i].m_NumClaimsLeft <= 0 ? true:false;
            var expiration:Number = PendingReward.m_Claims[i].m_ExpireDate;
            if (expiration == 0)
            {
            ...
            item.m_Origin = PendingReward.m_Claims[i].m_OriginDescription;
            
            m_ItemsArray.push(item);
        }
        
        m_ClaimAllButton.disabled = (PendingReward.m_Claims.length == 0);
        m_ClaimButton.disabled = true;
        m_DeleteButton.disabled = true;
        m_SelectedID = 0;
    ...
    private function ResponseButtonEventHandler(event:Object):Void
    {
        switch (event.target)
        {
            ...
            case m_ClaimButton:     PendingReward.ClaimItem(m_SelectedID);
                                    break;
                                    
            case m_ClaimAllButton:  PendingReward.ClaimAllItems();
        }
    ...
    private function SlotPromptResponse(response:String, id:Number):Void
    {
        if (response == PromptWindow.RESPONSE_OK)
        {
            ...
            {
                case m_DeleteButton:    PendingReward.DeleteClaimItem(id);
                                        break;
            }
            m_SelectedID = 0;
            m_ClaimList.ClearSelection();
    ...
    private function DisableMainWindow(value:Boolean):Void
    {
        ...
        */
        ...
        m_ClaimAllButton.disabled = value || (PendingReward.m_Claims.length == 0);
    }
}
