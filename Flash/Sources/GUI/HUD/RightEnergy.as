...
function onLoad() 
{
	...
	//almost all other code should be done in EnergyBars, as it will be the same for the left weapon!
...
{	
	...
	if (edit)
	{
		...
		{
			var scaleDV:DistributedValue = DistributedValue.Create("RightEnergyScale");
...
function SlotEditMaskPressed()
{
	...
	scale *= DistributedValue.GetDValue("RightEnergyScale", 100) / 100;
...
function SlotEditMaskReleased()
{
	...
	var newX:DistributedValue = DistributedValue.Create( "RightEnergyX" );
	var newY:DistributedValue = DistributedValue.Create( "RightEnergyY" );	
